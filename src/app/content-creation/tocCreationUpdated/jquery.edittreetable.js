...
(function($){
	$.fn.bstreetable = function(options){
        ...
        /*render data*/
        var dom_addFirstLevel = $("<div class='tt-operation m-b-sm'></div>").append($("<button class='btn btn-primary btn-sm j-resetData' ><i class='fa fa-level-down'></i>&nbsp;Reset Data</button>&nbsp;&nbsp;&nbsp;<button id='genearateJsonButton' class='btn btn-primary btn-sm j-genearateJson' ><i class='fa fa-level-down'></i>&nbsp;Save TOC json</button>&nbsp;&nbsp;&nbsp;<button id='publishJsonButton' class='btn btn-primary btn-sm j-publishJson' ><i class='fa fa-level-down'></i>&nbsp;Publish TOC json</button>"));//
    ...
    //     }
        ...
        element.delegate(".j-genearateJson","click",function(){
         ...
         element.delegate(".j-publishJson","click",function(){
           ...
           // var final_json = []
           final_json = getChildsData("0");
           temp =JSON.stringify(final_json);
        ...
        element.delegate(".form-control","blur",function(){
            ...
            parentUl.find(".form-control").each(function(){
                if($(this).attr("id").match("itemType")){
                   ...
                   // data[$(this).attr("name")]=$(this).val();    
                    ...
                    tocdata[tempid].label = $(this).val();    
                    $("#displayitemTypeLabel_"+tempid).text($(this).val());
                   // $("#displaynameLabel_"+data.id).text($(this).val());
                    //$("#displaynameLabel_"+data.id).text("lastorg");
            ...
            parentUl.find(".form-control").each(function(){
                if($(this).attr("id").match("metadata")){
                   ...
                   // data[$(this).attr("name")]=$(this).val();    
                    //alert("presentId:"+data.id);
                    tocdata[tempid].metadata = eval($(this).val());    
    ...
    fileEntry.createWriter(function(fileWriter) {
      ...
      fileWriter.write(blob);
		...
		/*function loadNode(loadElement,parentNode){
			...
			var curElement = loadElement.parent().parent().parent();
            var curLevel = loadElement.parent().attr("data-level")-0+1;            
        	if(parentNode&&parentNode.id){
                for(var i=0;i<settings.data.length;i++){
    	        	var row = settings.data[i];
    	        	//render first level row while row.pid equals 0 or null or undefined
    	        	if(row.pid*1==parentNode.id){
    	        		generateTreeNode(curElement,row,curLevel,false,false);
                        //cache treenode 
                        treeData[row.id] = row;
    	        	}
        ...
        function generateTreeNode(curElement,row,curLevel,isPrepend,isOnload){
            try{
                ...
                var metaData = JSON.stringify(row['metadata']);
                ...
                for(let i=_index+1; i<= presentId;i++){
			        ...
			        if(tmp_val==_index){
                       ...
                       //curElement.parents(".class-level-"+(parentUl.attr("data-level")-1)).children("ul").attr("data-innercode");
                        ...
                        temp_data.label = tocdata[i+''].itemType; 
